...
//# sourceMappingURL=data:application/json;base64,${Buffer.from(JSON.stringify(template.map)).toString('base64')}`;
  }
  let cssInJS = '';
...
document.body.appendChild(el);}());`;
  ...
  const moduleCode = `
  import script from 'data:text/javascript;base64,${Buffer.from(script.content).toString('base64')}';
  import {render} from 'data:text/javascript;base64,${Buffer.from(template.code).toString('base64')}';
...
function reactTransform(source, url) {
  ...
  const res = esbuild.transformSync(source, {loader: ext.slice(1), sourcemap: true});
  if(res.map) {
