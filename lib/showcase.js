...
async function activeClient(server, options, events) {
  ...
  try {
    ...
    await sleep(delay != null ? delay : 100); // wait 100ms for execute js;
...
class Showcase {
  ...
  async showComponent({
    path = null,
    content = null,
    module = this.options.type === 'vue' ? 'app.vue' : 'app.jsx',
    ...
    let {servedir} = this.options.serverOptions;

    if(path && !servedir) {
      servedir = require('path').dirname(path);
    }
    this.options.serverOptions.servedir = servedir;
    const {htmlTemplate, serverOptions} = content !== null ? types[type].fromText(content, module) : types[type].fromFile(path, servedir);
