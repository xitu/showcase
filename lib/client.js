...
class Client {
      ...
      const promise = new Promise((resolve) => {
        ...
        rl.on('line', async (input) => {
          if(input === '') {
            ...
            const task = this.dispatchEvent(args[0], callback, ...args.slice(1));
            tasks.push(task);
          }
        });
      });
  ...
  /* async */
  async dispatchEvent(event, onEvent = null, ...args) {
    const handler = replCommands[event];
    if(handler) {
      await handler(this.page, ...args);
      if(onEvent) await onEvent(this, this.page);
    } else {
      throw new ReferenceError(`unknow event ${event}`);
    }
  }

  isEvent(eventType) {
    return eventType in replCommands;
  }
}
